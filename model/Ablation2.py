...
class Intra_EPI_Transformer(nn.Module):
    def __init__(self, channels, num_heads=4, bias=False):
        ...
        # self.qkv_dwconv = snake_conv(channels * 3)
...
class Inter_EPI_Transformer(nn.Module):
    def __init__(self,
        ...
        # self.qkv_dwconv = snake_conv(embed_dim * 3)
...
class Fusion_Block(nn.Module):
    ...
    def forward(self, warp):
        b, c, h, w = warp.shape

        x_1, x_2 = warp.chunk(2, dim=1)
...
class get_model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.fusion_type = 'cat'  # 'cross_attention' or 'cat'

        # Feature Alignment
        if self.fusion_type == 'cross_attention':
            self.cross_fusion = TransformerBlock(self.channels)
        elif self.fusion_type == 'cat':
            self.cross_fusion = nn.Sequential(
                nn.Conv2d(self.channels * 2, self.channels, 1, bias=False),
                nn.LeakyReLU(0.1, inplace=True),
                nn.Conv2d(self.channels, self.channels, 3, 1, 1, bias=False),
            )
        else:
    ...
    def forward(self, warping_Light_Field, reference_Light_Field):
        ...
        # Feature alignment
        if self.fusion_type == 'cross_attention':
            warping_fea = self.cross_fusion(warp_fea, ref_fea)
        else:
            warping_fea = self.cross_fusion(torch.cat([warp_fea, ref_fea], dim=1))
