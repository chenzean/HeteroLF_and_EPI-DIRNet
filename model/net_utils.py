...
from model.gilbert2d import gilbert2d

import selective_scan_cuda_oflex_rh


def hilbert_curve_scale(H: int = 64, W: int = 64, device=None, gilbert_returns_xy: bool = True):
    """
    生成 2D 希尔伯特/吉尔伯特曲线的线性索引，用于将 [H, W] 的网格
    以希尔伯特顺序展平为长度 H*W 的一维序列。

    参数:
        H, W: 特征图高宽
        device: 返回索引要放到的设备（如 x.device）
        gilbert_returns_xy: 你的 gilbert2d 是否返回 (x, y)
                            - 若 True: 认为返回 (x, y)
                            - 若 False: 认为返回 (y, x)

    返回:
        {'hilbert_curve_small_scale': idx} ，其中 idx 形状为 [H*W] 的 LongTensor
    """
    # 1) 生成 2D 曲线路径：注意 width 对应 W，height 对应 H
    path = list(gilbert2d(width=W, height=H))  # 正确写法：width=W, height=H

    ij = torch.tensor(path, dtype=torch.long, device=device)

    # 2) 统一成 (y, x)
    if gilbert_returns_xy:
        # path 是 (x, y) => 交换到 (y, x)
        y, x = ij[:, 1], ij[:, 0]
    else:
        # path 已是 (y, x)
    ...
    return {'hilbert_curve_small_scale': idx}
